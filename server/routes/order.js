...
router.get('/', requireAdmin, async (req, res) => {
  try {
    await connectDB();
    const orders = await Order.findAll(true); // Admin için çözülmüş
    res.json({
      success: true,
      orders
    });
...
router.get('/:id', requireAdmin, async (req, res) => {
  try {
    await connectDB();
    const order = await Order.findById(req.params.id, true); // Admin için çözülmüş
    if (!order) {
...
router.delete('/all', requireAdmin, async (req, res) => {
  try {
    await connectDB();
    const deletedCount = await Order.deleteAll();
